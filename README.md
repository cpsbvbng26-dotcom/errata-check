...
いちばん効くのは 完全性 と 未解決 です。ほかは書き間違いを捕まえます。この二つは、
書き手が自分に都合よく直すことを捕まえます。

### 分野ごとに、紙面から決定的に確かめられるもの

引用の一致だけが検査ではありません。分野が違えば、紙面から機械で確かめられるものも違います。
...
| **本文だけに出る名** | 本文で名を挙げておきながら、**参考文献欄に無い**もの。年号つきの引用の形をとらない言及は、上の網に掛からない | 同上 |
...
「誤っていること」も宣言できます。`consistent = false` と書くと、
計算し直した `p` が印字と**合わないこと**のほうを検査します。正誤表が
...
内面は外から確かめられません。「読んだ」と宣言させても、それは証言であって検査では
ありません。

「どの箇所が、どの主張を支えているか」は違います。公開された主張であり、その本を持つ者
なら誰でも反証できます。だから宣言させるのは読書ではありません。指し示しのほうです。
...
`require_used` が効きます。本文で使っていない文献を並べて厚く見せることが、構造的に
できなくなります。
...
実装は独立で、それぞれのコードは見ていません。違いが一つあります。statcheck は
論文から統計量を**自動で抜き出します**。この道具は**宣言されたものだけ**を見ます。
抜き出しの誤りによる偽陽性が出ない代わりに、宣言し忘れた箇所は見ません。
どちらが良いという話ではなく、目的が違います。ここは網羅ではなく、
**正誤表に書いたことが本当かどうか**を押さえる道具です。

### 分布は自分で実装している

`p` を出すのに scipy は使いません。単一ファイルで配れなくなるためです。
正則化不完全ベータ・ガンマを Lentz の連分数と級数で書いてあります。

実装が正しいかどうかは、実装だけでは分かりません。公表されている統計数値表の
...
文字そのものは置き換えません。だから「引用が一字違う」は落ちるが、
「行が折り返された」では落ちません。

## この道具自身の検査

```
python3 tests/check_tool.py     # 74 項目
```
...
決めていない使い方を書く、引用の宣言を忘れる、使っていない参考文献を宣言する、本文だけに出る名を本文から消す、
その名を参考文献欄に置きます。
...
- PDF から取り出した文字列に依存します。取り出し方（pypdf）が変われば結果も変わる。
  合字や特殊な字形を使った箇所では、一致しないことがある
- 一次資料が画像だけの PDF なら、何も取り出せません。OCR は持たない
- 意味は見ません。引用が原文にあることは見ます。その引用が文脈を歪めて切り取られて
  いないかは見ない
- 網羅しません。宣言された箇所だけを見ます。書き落とした箇所は見ない
- 検定の再計算は、印字された値が内部で整合するかだけを見ます。その検定を使うべき
  だったか、前提が満たされているかは見ない
- `[[number]]` は任意のコマンドを実行します。信用できる宣言だけを走らせること
...
通る状態を壊して落ちることまで確かめる試験の作り方 ―― これらは著者の判断です。

検査が報告する数値は、すべて実行して得たものです。AI は著作者ではありません。いずれの
主張についても、責任は著者（根本卓哉）にあります。

このリポジトリのコミットは `Claude` 名義であり、末尾に作業セッションを示す
`Claude-Session:` トレーラが付きます。`git log --author=Claude` で辿れます。

Zenodo に登録した v0.1.0 と v0.2.0 のレコードには、この記載がありません。公開後に
ファイルは差し替えられないため、直せるのはこちら側だけです
...
[CONTRIBUTING.md](CONTRIBUTING.md) にあります。検査を足すときは、壊す試験も一緒に足します。
維持しているのは著者一人で、組織の後ろ盾はありません。範囲と後方互換の方針も同じ文書にあります。

---

## 出す先

JOSS（Journal of Open Source Software）に出します。論文は [`paper.md`](paper.md)、
手順と、通らない見込みは [`SUBMIT.md`](SUBMIT.md) にあります。結果はまだ出ていません。
